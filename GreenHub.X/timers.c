...
void configure_timer1(void) {
    ...
    PR1 = TIMER1_PERIODA; //perioda 0.5ms
    T1CONbits.TCS = 0; // 0 = Internal clock (FOSC/4)
    IPC0bits.T1IP = 1; //prioritet tajmera srednji 
...
void __attribute__((__interrupt__, __auto_psv__)) _T1Interrupt(void) {
    ...
    brojac_pola_ms++; // 0.5ms brojac
    IFS0bits.T1IF = 0; //clearujemo fleg
}
...
void configure_timer2(void) {
    ...
    IPC1bits.T2IP = 0; //prioritet tajmera 2 najmanji
...
void configure_timer3(void) {
    ...
    IPC1bits.T3IP = 2; //prioritet tajmera 3 najveci jer se najcesce desava
